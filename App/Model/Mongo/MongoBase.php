...
use App\Utility\Pool\MongoObject;
...
class MongoBase
{
    ...
    {
        //$this->mongoClient = PoolManager::getInstance()->getPool(MongoPool::class)->getObj();
        $this->mongoClient = PoolManager::getInstance()->getPool(MongoPool::class)->defer();//取出MONGO连接池
    ...
    public function __destruct()
    {
        ...
        if ($this->mongoClient instanceof MongoObject) {
            PoolManager::getInstance()->getPool(MongoPool::class)->recycleObj($this->mongoClient);
    ...
    public function db($dbname)
    {
        ...
        $this->database = $dbname[0];
        $this->collection = $dbname[1];
        return $this;
    }
      ...
      $wtimeout = 1000)
    {
        try {
            ...
            $result = $this->mongoClient->executeBulkWrite($dbc, $bulk, $wc);
            $this->result = $result;
            //增加几条
    ...
    public function find($id = null)
    {
        ...
        $dbc = $this->database . '.' . $this->collection;
        $documents = $this->mongoClient->executeQuery($dbc, $query);
    ...
    {
        try {
            $cursor = $this->mongoClient->executeCommand($db, $commands);
            $this->result = $cursor;
            return $cursor;
        } catch (\Exception $e) {
            $this->showError($e);
